"""Notebook helper implementations migrated from `Test_pilot/test_nb_stim_analysis.py`.
...
def plot_gridspec_subplot(fig, gs, position, time, avg_data, sem_data, title, color='black', 
                        ...
                        n_neurons=None, n_trials=None):
    """Create a subplot with mean, SEM shading, and stimulus onset markers."""
    ax = fig.add_subplot(gs[position])
    label = f'Mean (n={n_neurons}, trials={n_trials})' if n_neurons is not None and n_trials is not None else 'Mean'
    ax.plot(time, avg_data, color=color, label=label)
...
def plot_gridspec_superimposed(fig, gs, position, time, avg_data1, sem_data1, label1, color1, 
                             ...
                             stim1_time=0, stim2_time=None, line_mode='all', stim_style='bar',
                             n_neurons1=None, n_trials1=None, n_neurons2=None, n_trials2=None):
    """Create a subplot with two superimposed traces."""
    ax = fig.add_subplot(gs[position])
    label1 = f'{label1} (n={n_neurons1}, trials={n_trials1})' if n_neurons1 is not None and n_trials1 is not None else label1
    label2 = f'{label2} (n={n_neurons2}, trials={n_trials2})' if n_neurons2 is not None and n_trials2 is not None else label2
...
def calculate_metrics(neu_seq, mask, labels):
    """Calculate average, SEM, and heatmap data for given mask, including neuron and trial counts."""
    exc_mask = labels == -1
    inh_mask = labels == 1
    n_trials = np.sum(mask)
    n_neurons_all = neu_seq.shape[1]
    n_neurons_exc = np.sum(exc_mask)
    n_neurons_inh = np.sum(inh_mask)
    return {
        ...
        'n_trials_exc': n_trials,
        'n_neurons_inh': n_neurons_inh,
        'n_trials_inh': n_trials
    }
...
def main(neural_trials_list, labels_list, save_path=None):
    ...
    neu_seq, neu_time, trial_type, block_type, isi, decision, labels, pooled_outcomes = pool_session_data(
    ...
    axes_group1 = [
        plot_gridspec_subplot(fig, gs, (0, 0), neu_time, metrics_short['avg_all'], metrics_short['sem_all'],
                            ...
                            n_neurons=metrics_short['n_neurons_all'], n_trials=metrics_short['n_trials_all']),
        plot_gridspec_subplot(fig, gs, (0, 1), neu_time, metrics_short['avg_exc'], metrics_short['sem_exc'],
                            ...
                            stim1_time=0, stim2_time=sec_stim_time_short, line_mode='all',
                            n_neurons=metrics_short['n_neurons_exc'], n_trials=metrics_short['n_trials_exc']),
        plot_gridspec_subplot(fig, gs, (0, 2), neu_time, metrics_short['avg_inh'], metrics_short['sem_inh'],
                            ...
                            stim1_time=0, stim2_time=sec_stim_time_short, line_mode='all',
                            n_neurons=metrics_short['n_neurons_inh'], n_trials=metrics_short['n_trials_inh']),
        plot_gridspec_superimposed(fig, gs, (0, 3), neu_time,
                                 ...
                                 stim1_time=0, stim2_time=sec_stim_time_short, line_mode='all',
                                 n_neurons1=metrics_short['n_neurons_exc'], n_trials1=metrics_short['n_trials_exc'],
                                 n_neurons2=metrics_short['n_neurons_inh'], n_trials2=metrics_short['n_trials_inh'])
    ]
    ...
    axes_group2 = [
        plot_gridspec_subplot(fig, gs, (2, 0), neu_time, metrics_long['avg_all'], metrics_long['sem_all'],
                            ...
                            n_neurons=metrics_long['n_neurons_all'], n_trials=metrics_long['n_trials_all']),
        plot_gridspec_subplot(fig, gs, (2, 1), neu_time, metrics_long['avg_exc'], metrics_long['sem_exc'],
                            title='Average dF/F excitatory neurons around first stimulus onset\nlong trials', color='blue',
                            stim1_time=0, stim2_time=sec_stim_time_long, line_mode='all',
                            n_neurons=metrics_long['n_neurons_exc'], n_trials=metrics_long['n_trials_exc']),
        plot_gridspec_subplot(fig, gs, (2, 2), neu_time, metrics_long['avg_inh'], metrics_long['sem_inh'],
                            title='Average dF/F inhibitory neurons around first stimulus onset\nlong trials', color='red',
                            stim1_time=0, stim2_time=sec_stim_time_long, line_mode='all',
                            n_neurons=metrics_long['n_neurons_inh'], n_trials=metrics_long['n_trials_inh']),
        plot_gridspec_superimposed(fig, gs, (2, 3), neu_time,
                                 ...
                                 title='Superimposed dF/F excitatory and inhibitory neurons around first stimulus onset\nlong trials',
                                 stim1_time=0, stim2_time=sec_stim_time_long, line_mode='all',
                                 n_neurons1=metrics_long['n_neurons_exc'], n_trials1=metrics_long['n_trials_exc'],
                                 n_neurons2=metrics_long['n_neurons_inh'], n_trials2=metrics_long['n_trials_inh'])
    ]
        ...
        for trial_key, row in row_starts.items():
            ...
            axes_group = [
                plot_gridspec_subplot(fig, gs, (row, 0), neu_time, metrics['avg_all'], metrics['sem_all'],
                                    ...
                                    n_neurons=metrics['n_neurons_all'], n_trials=metrics['n_trials_all']),
                plot_gridspec_subplot(fig, gs, (row, 1), neu_time, metrics['avg_exc'], metrics['sem_exc'],
                                    ...
                                    n_neurons=metrics['n_neurons_exc'], n_trials=metrics['n_trials_exc']),
                plot_gridspec_subplot(fig, gs, (row, 2), neu_time, metrics['avg_inh'], metrics['sem_inh'],
                                    ...
                                    n_neurons=metrics['n_neurons_inh'], n_trials=metrics['n_trials_inh']),
                plot_gridspec_superimposed(fig, gs, (row, 3), neu_time,
                                         ...
                                         n_neurons1=metrics['n_neurons_exc'], n_trials1=metrics['n_trials_exc'],
                                         n_neurons2=metrics['n_neurons_inh'], n_trials2=metrics['n_trials_inh'])
            ]
    ...
    reward_mask = outcomes == 'reward'
    punish_mask = outcomes == 'punish'
    ...
    for title, mask1, mask2, row in conditions_short_block:
        ...
        axes_group = [
            plot_gridspec_superimposed(fig, gs, (row, 0), neu_time,
                                     metrics1['avg_all'], metrics1['sem_all'], 'Short trials', 'orange',
                                     metrics2['avg_all'], metrics2['sem_all'], 'Long trials', 'purple',
                                     title=f'All neurons: {title}',
                                     stim1_time=0, stim2_time=avg_sec_stim_time, line_mode='all',
                                     n_neurons1=metrics1['n_neurons_all'], n_trials1=metrics1['n_trials_all'],
                                     n_neurons2=metrics2['n_neurons_all'], n_trials2=metrics2['n_trials_all']),
            plot_gridspec_superimposed(fig, gs, (row, 1), neu_time,
                                     metrics1['avg_exc'], metrics1['sem_exc'], 'Short trials', 'orange',
                                     metrics2['avg_exc'], metrics2['sem_exc'], 'Long trials', 'purple',
                                     title=f'Excitatory neurons: {title}',
                                     stim1_time=0, stim2_time=avg_sec_stim_time, line_mode='all',
                                     n_neurons1=metrics1['n_neurons_exc'], n_trials1=metrics1['n_trials_exc'],
                                     n_neurons2=metrics2['n_neurons_exc'], n_trials2=metrics2['n_trials_exc']),
            plot_gridspec_superimposed(fig, gs, (row, 2), neu_time,
                                     metrics1['avg_inh'], metrics1['sem_inh'], 'Short trials', 'orange',
                                     metrics2['avg_inh'], metrics2['sem_inh'], 'Long trials', 'purple',
                                     title=f'Inhibitory neurons: {title}',
                                     stim1_time=0, stim2_time=avg_sec_stim_time, line_mode='all',
                                     n_neurons1=metrics1['n_neurons_inh'], n_trials1=metrics1['n_trials_inh'],
                                     n_neurons2=metrics2['n_neurons_inh'], n_trials2=metrics2['n_trials_inh']),
            plot_gridspec_superimposed(fig, gs, (row, 3), neu_time,
                                     metrics1['avg_exc'], metrics1['sem_exc'], 'Exc - Short', 'blue',
                                     metrics1['avg_inh'], metrics1['sem_inh'], 'Inh - Short', 'red',
                                     title=f'Exc vs Inh: {title.split(" vs ")[0]}',
                                     stim1_time=0, stim2_time=avg_sec_stim_time, line_mode='all',
                                     n_neurons1=metrics1['n_neurons_exc'], n_trials1=metrics1['n_trials_exc'],
                                     n_neurons2=metrics1['n_neurons_inh'], n_trials2=metrics1['n_trials_inh'])
        ]
    ...
    # Long majority block analysis (rows 34-39)
    ...
    # Block-based analysis - aligned on second stimulus (rows 40-45)
    neu_seq, neu_time, trial_type, block_type, isi, decision, labels, outcomes = pool_session_data(
        neural_trials_list, labels_list, 'stim_seq', l_frames, r_frames, indices=2)
    ...
    # Short majority block analysis - aligned on second stimulus (rows 40-45)
    ...
    # Long majority block analysis - aligned on second stimulus (rows 46-51)
    for title, mask1, mask2, row in [
        ('all short trials vs all long trials in long block', 
    ...
    if save_path is not None:
        figures_dir = os.path.join(save_path, 'Figures', 'stim')
        os.makedirs(figures_dir, exist_ok=True)
        plt.savefig(os.path.join(figures_dir, 'neural_respnes.pdf'))
    else:
        plt.savefig('neural_respnes.pdf')
    plt.show()
