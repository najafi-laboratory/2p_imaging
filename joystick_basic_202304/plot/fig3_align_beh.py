...
def plot_beh_outcome(ax, neural_trials):
    reward = np.array([not np.isnan(neural_trials[str(i)]['time_reward'][0])
              for i in range(len(neural_trials))])
    punish = np.array([not np.isnan(neural_trials[str(i)]['time_punish'][0])
              for i in range(len(neural_trials))])
...
def plot_align_pos_reward(ax, neural_trials):
    [align_data, align_time, _, _] = align_pos(neural_trials, 'time_reward')
...
def plot_align_pos_vis1(ax, neural_trials):
    [align_data, align_time, _, outcome] = align_pos(neural_trials, 'time_vis1')
    ...
    adjust_layout_trajectory(ax)
    ax.set_xlabel('time since first stimuli (ms)')
    ax.set_xlim([np.min(align_time), np.max(align_time)])
    ax.set_ylim([lower, upper + 0.1*(upper-lower)])
    ax.set_title(
        'first stimuli aligned trajectories for {} trials'.format(
        align_data.shape[0]))


# trajectory aligned at second stimuli.

def plot_align_pos_vis2(ax, neural_trials):
    [align_data, align_time, _, outcome] = align_pos(neural_trials, 'time_vis2')
    mean_reward = np.mean(align_data[outcome==1], axis=0)
    mean_punish = np.mean(align_data[outcome==-1], axis=0)
    sem_reward  = sem(align_data[outcome==1], axis=0)
    sem_punish  = sem(align_data[outcome==-1], axis=0)
    ...
    ax.set_ylim([lower, upper + 0.1*(upper-lower)])
    ax.set_title(
        'second stimuli aligned trajectories for {} trials'.format(
        align_data.shape[0]))
