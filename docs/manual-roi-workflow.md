...
If the session was generated by `utils_2p.preprocessing_qc_pipeline`, reuse its
manifest:
...
### Build `data.bin` when there is no manifest

Most older processed sessions will not have a new-pipeline `manifest.json`.
Create a minimal temporary manifest that points at the existing processed
session and the raw TIFF folder, then call the same `prebuild()` function.
...
repo = Path("/storage/home/hcoda1/3/grubin6/2p_imaging")
raw_tiffs = Path("/path/to/raw_tiff_folder")
processed_session = Path("/path/to/processed/session")
manifest = processed_session / "manual_roi_prebuild_manifest.json"

manifest.write_text(
    json.dumps(
        {
            "pipeline": {
                "processing_root": str(repo / "2p_processing_pipeline_202401"),
                "suite2p_binary_batch_size": 5000,
                "suite2p_registration_batch_size": 500,
                "suite2p_extraction_batch_size": 500,
                "keep_suite2p_bin": True,
                "fast_disk": str(processed_session / ".suite2p_fast_disk"),
            },
            "sessions": [
                {
                    "name": processed_session.name,
                    "raw_path": str(raw_tiffs),
                    "output_path": str(processed_session),
...
Set `target_structure` to the same class of Suite2p config that was used for
the session, usually `neuron` or `dendrite`. The example above is for a
