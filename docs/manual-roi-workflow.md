...
!!! note
    New sessions should not create separate `qc_results/` or
    `manual_qc_results/` directories by default. Those folder layouts are
    legacy compatibility targets for older workflows. For new sessions, review
    and label ROIs in the interactive HTML reviewer whenever possible.
...
For future pipeline jobs generated by `utils_2p.processing_pipeline`,
