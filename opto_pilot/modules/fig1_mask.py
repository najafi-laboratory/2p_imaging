...
def get_labeled_masks_img_test(masks, labels, cate):
    neuron_idx = np.where(labels == cate)[0] + 1
    labeled_masks_img = np.zeros(
...
class plotter_all_masks:
    ...
    # functional channel.
    def func_effective(self, ax, img, with_mask=True):
        if img == 'mean':
            f = self.mean_func
            t = 'significant ROIs mean projection'
        if img == 'max':
            f = self.max_func
            t = 'significant ROIs max projection'
        func_img = np.zeros(
            (f.shape[0], f.shape[1], 3), dtype='int32')
        func_img[:, :, 1] = adjust_contrast(f)
        func_img = adjust_contrast(func_img)
        if with_mask:
            # x_all, y_all = np.where(find_boundaries(get_labeled_masks_img(
            #     self.masks, self.labels, 1)[:, :, 0]))
            x_all, y_all = np.where(find_boundaries(get_labeled_masks_img_test(self.masks, self.labels, 1)))
            for x,y in zip(x_all, y_all):
                func_img[x,y,:] = np.array([255,255,255])
        ax.matshow(func_img)
        adjust_layout(ax)
