...
- **RoMa's fused local-correlation kernel is Linux-only, and the check is on `sys.platform`, not on whether CUDA exists.** Both macOS and Windows fall back to a pure-PyTorch implementation and warn once per `ConvRefiner` the model builds. Results are unaffected — the fallback computes the same correlation with a Python loop over the batch — so only speed suffers. The notebook's import cell filters the warning by message rather than by platform, which keeps it correct on both and a no-op on Linux. Roughly, fastest to slowest: Linux+CUDA (kernel and CUDA) > Windows+CUDA (CUDA, no kernel) > macOS+MPS (neither) > CPU. The cluster is the only place the fast path is available.
- **`DEVICE` requests a GPU.** On a Mac that selects MPS, verified working on torch 2.11; an older torch missing an MPS operator will fail where CPU would merely have been slow. `roicat.helpers.set_device(use_GPU=False)` is the fallback.
