...
def network(root: Path, patch: int):
    """Create the bundled SRDTrans network architecture."""
    # Import only the upstream network. Do not use its data or CLI code.
...
def denoised_stack(files: list[Path], count: int, output: Path, gpu: str,
                   patch: int, root: Path, model: Path) -> Iterator[Path]:
    """Yield a temporary direct-inference result and remove it afterward."""
    root, model, output = root.resolve(), model.resolve(), output.resolve()
    if not (root / "SRDTrans" / "__init__.py").is_file():
        raise FileNotFoundError(f"SRDTrans network package not found in {root}")
    ...
    with tempfile.TemporaryDirectory(prefix="srdtrans_", dir=output) as temporary:
        ...
        torch, network, devices = _model(root, model, patch, gpu)
