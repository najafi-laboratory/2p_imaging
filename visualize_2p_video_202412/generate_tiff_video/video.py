...
from tiff_io import frame_counts, frame_rate, pages
...
def process(files: list[Path], duration: float, output: Path, rolling: int,
            speed: float, gpu: str, patch: int, model: Path) -> tuple[Path, float, int]:
    ...
    with denoised_stack(files, count, output, gpu, patch, model) as denoised:
