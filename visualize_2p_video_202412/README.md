...
```powershell
python -m generate_tiff_video data --duration 30 --rolling-average 8 --speed 1
```
...
```powershell
python -m generate_tiff_video data `
  --duration 30 `
  --rolling-average 8 `
  --speed 1 `
  --output output
...
| `--model` | `trained_model` | Folder containing checkpoint files. |
...
```text
generate_tiff_video/        TIFF-to-video workflow
  __main__.py               Video command entry point
  cli.py                    Video arguments and validation
  srdtrans.py               Checkpoint loading and tiled CUDA inference
  video.py                  Contrast, smoothing, timing, and MP4 encoding
train_model/                Denoising-model training workflow
  __main__.py               Training command entry point
  train.py                  Self-supervised training
  srdtrans_runtime.py       Network construction and CUDA setup
srdtrans_model/             Flat SRDTrans architecture package
tiff_io.py                  TIFF discovery, metadata, and streaming
trained_model/              Default checkpoint location
```

The training folder owns the SRDTrans runtime used by both workflows. TIFF I/O
remains a small root module, while the model architecture stays isolated from
the workflow code.
