...
from .srdtrans_runtime import network, runtime
from tiff_io import find_tiffs, frame_counts, pages
...
def train(data: Path, output: Path, frames: int, epochs: int, patches: int,
          patch: int, batch: int, gpu: str, learning_rate: float) -> Path:
...
def main() -> None:
    ...
    train(args.data, args.output, args.frames, args.epochs, args.patches_per_epoch,
          args.patch, args.batch_size, args.gpu, args.learning_rate)
