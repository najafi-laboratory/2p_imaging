...
def get_multi_sess_neu_trial_perception(
        neu_cate,
        alignment,
        trial_idx=None,
        mean_sem=True,
    ...
    if trial_idx != None:
        for i in range(len(neu_cate)):
            ...
            trial_type.append(alignment['list_trial_type'][i][trial_idx[i]])
            isi.append(alignment['list_isi'][i][trial_idx[i]])
            decision.append(alignment['list_decision'][i][trial_idx[i]])
            outcome.append(alignment['list_outcome'][i][trial_idx[i]])
    else: 
        for i in range(len(neu_cate)):
            neu.append(neu_cate[i])
            stim_seq.append(alignment['list_stim_seq'][i])
            stim_value.append(alignment['list_stim_value'][i])
            trial_type.append(alignment['list_trial_type'][i])
            isi.append(alignment['list_isi'][i])
            decision.append(alignment['list_decision'][i])
            outcome.append(alignment['list_outcome'][i])
    # get numbers.
    ...
    if mean_sem:
        ...
        return [mean, sem, stim_seq, stim_value, trial_type, isi, decision, outcome], [n_trials, n_neurons]
    # return single trial response.
    else:
        return [neu, stim_seq, stim_value, trial_type, isi, decision, outcome], [n_trials, n_neurons]

# for multi session settings find trials based on stim_labels.
def get_multi_sess_neu_trial_decision(
        neu_cate,
        alignment,
        trial_idx=None,
        mean_sem=True,
    ...
    if trial_idx != None:
        for i in range(len(neu_cate)):
            neu.append(neu_cate[i][trial_idx[i],:,:])
            direction.append(alignment['list_direction'][i][trial_idx[i]])
            correction.append(alignment['list_correction'][i][trial_idx[i]])
            lick_type.append(alignment['list_lick_type'][i][trial_idx[i]])
    else: 
        for i in range(len(neu_cate)):
            neu.append(neu_cate[i])
            direction.append(alignment['list_direction'][i])
            correction.append(alignment['list_correction'][i])
            lick_type.append(alignment['list_lick_type'][i])
    # get numbers.
    n_trials = np.nansum([n.shape[0] for n in neu])
    n_neurons = np.nansum([n.shape[1] for n in neu])
    # compute trial average and concatenate.
    if mean_sem:
        ...
        mean = np.concatenate(mean, axis=0)
        sem  = np.concatenate(sem, axis=0)
        return [mean, sem, direction, correction, lick_type], [n_trials, n_neurons]
    # return single trial response.
    else:
        return [neu, direction, correction, lick_type], [n_trials, n_neurons]
    
# find neuron category and trial data.
def get_neu_trial(
        alignment, list_labels, list_significance,
        trial_idx=None, mean_sem=True,
        cate=None, roi_id=None, mode=None,
        ):
    if cate != None:
        colors = get_roi_label_color(cate=cate)
        neu_labels = np.concatenate([
            list_labels[i][np.in1d(list_labels[i],cate)*list_significance[i]['r_all']]
            for i in range(len(list_labels))])
        neu_sig = np.concatenate([
            list_significance[i]['r_all'][np.in1d(list_labels[i],cate)]
            for i in range(len(list_significance))])
        neu_cate = [
            alignment['list_neu_seq'][i][:,np.in1d(list_labels[i],cate)*list_significance[i]['r_all'],:]
            for i in range(len(alignment['list_neu_seq']))]
    if roi_id != None:
        colors = get_roi_label_color(list_labels, roi_id=roi_id)
        neu_labels = None
        neu_cate = [np.expand_dims(alignment['list_neu_seq'][0][:,roi_id,:], axis=1)]
    if mode == 'perception':
        neu_trial, neu_num = get_multi_sess_neu_trial_perception(
            neu_cate, alignment,
            trial_idx=trial_idx, mean_sem=mean_sem)
    if mode == 'lick':
        neu_trial, neu_num = get_multi_sess_neu_trial_decision(
            neu_cate, alignment,
            trial_idx=trial_idx, mean_sem=mean_sem)
