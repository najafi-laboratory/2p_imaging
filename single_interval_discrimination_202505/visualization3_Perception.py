...
from plot.fig4_perception import plotter_main
...
def run(session_config_list, smooth):
    ...
    if (np.sum(idx_target_sess)==0) or (np.sum(idx_target_sess)==1 and not idx_target_sess[0]):
        ...
        def plot_perception():
            title = 'resonse to general sensory input'
            print('-----------------------------------------------')
            print(title)
            filename = 'perception01_perception'
            n_row = 2
            n_col = 5
            fig = plt.figure(figsize=(n_col*size_scale, n_row*size_scale), layout='tight')
            gs = GridSpec(n_row, n_col, figure=fig)
            axs_all = [[plt.subplot(gs[0, i]) for i in range(4)]]
            axs_all+= [[plt.subplot(gs[1, i]) for i in range(4)]]
            plotter.perception(axs_all)
