...
def compute_glm_weights(features, neural, ops, alpha=1):
    ...
    # 1. Determine the shortest length between features and neural data
    min_len = min(features.shape[1], neural.shape[1])
    
    # 2. Crop both to the same size (taking the first min_len frames)
    features = features[:, :min_len]
    neural = neural[:, :min_len]
    
    # 3. Proceed with normalization and fitting
