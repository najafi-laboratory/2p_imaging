...
def save(ops, name, data):
    f = h5py.File(os.path.join(ops['save_path0'], 'qc_results', name+'.h5'), 'w')
    f[name] = data
    f.close()
def save_manual(ops, name, data):
    f = h5py.File(os.path.join(ops['save_path0'], 'manual_qc_results', name+'.h5'), 'w')
    f[name] = data
    f.close()
...
def run_manual(
    ...
    print('=========== dff trace normalization ===========')
    print('===============================================')
    print('Reading fluorescence signals after manual quality control')
    fluo = np.load(
        os.path.join(ops['save_path0'], 'manual_qc_results', 'fluo.npy'),
        allow_pickle=True)
    neuropil = np.load(
        os.path.join(ops['save_path0'], 'manual_qc_results', 'neuropil.npy'),
        allow_pickle=True)
    print('Running baseline subtraction and normalization')
    dff = get_dff(ops, fluo, neuropil, norm)

    save_manual(ops, 'dff', dff)
    print("DFF Results saved under name 'dff'")
    ...
    if len(taus) > 1:
        ...
        save_manual(ops, 'spikes', spikes)
        print("Spike traces saved under name 'spikes'")

        save_manual(ops, 'smoothed', smoothed)
        print("De-noised DFF data saved under name 'smoothed'")
