...
def trial_split(
    ...
    for i in range(len(start)):
        ...
        end_idx_dff = np.where(time_neuro < end[i])[
            0][-1] if end[i] != -1 else -1
        ...
        end_idx_vol = np.where(vol_time < end[i])[
            0][-1] if end[i] != -1 else -1
        neural_trials[str(
            i)]['vol_stim'] = label_stim[start_idx_vol:end_idx_vol]
...
def trial_label(
    ...
    for i in range(np.min([len(neural_trials), len(bpod_sess_data['trial_types'])])):
        ...
        neural_trials[str(i)]['trial_lick'] = bpod_sess_data[
            'trial_lick'][i]
        neural_trials[str(i)]['trial_lick'][0,
                                            :] += neural_trials[str(i)]['vol_time'][0]
