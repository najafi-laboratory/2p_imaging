...
def pick_trial(
    ...
    #idx7 = np.isin(stim_labels[:,2], img_seq_label)       if opto_types       else np.ones_like(stim_labels[:,7])
...
def sub_sampling_trial(neu_seq, samping_size=3, sampling_times=252):
    print('shape trials: ', neu_seq.shape)
...
def get_multi_sess_neu_trial(
    ...
    print('mode in get_multi_sess_neu_trial in utils: ',mode)
    ...
    for i, labels in enumerate(list_stim_labels):
        if mode == 1:
            ...
            # added but double check not to cause errors
            idx = idx[:len(neu_cate[i])]
        elif mode == 2:
            idx = trial_idx[i]
            idx = idx[:len(neu_cate[i])]
        elif mode == 3:
            ...
            continue
        #print('all lens:', len(neu), len(neu_cate), len(neu_cate[i]), len(idx))
        neu.append(neu_cate[i][idx,...])
        print(list_stim_seq[i].shape)
        stim_seq.append(list_stim_seq[i][idx,...] )
        #print('num_trial', len(idx))
    ...
    print('shape trials: ', n_trials)
    print('shape neurons: ', n_neurons)
    if n_trials<50:
        print(pre_isi)
        print(post_isi)
    if mean_sem:
        if sub_sampling:
            ...
            #print('please: ', np.concatenate(stim_seq,     axis=0).shape)
            stim_seq     = np.nanmean(np.concatenate(stim_seq,     axis=0), axis=0)
            camera_pupil = np.nanmean(np.concatenate(camera_pupil, axis=0), axis=0)
...
def get_odd_stim_prepost_idx(stim_labels):
    ...
    #print(stim_labels[idx_post_short,0]-stim_labels[idx_pre_short,1])
    return idx_pre_short, idx_pre_long, idx_post_short, idx_post_long

# get index for pre/post short/long oddball.
def get_stim_seq_num_idx(stim_labels):
    idx_1 = (stim_labels[:,8]==0) 
    idx_2 = (stim_labels[:,8]==1) 
    idx_3 = (stim_labels[:,8]==2) 
    idx_4 = (stim_labels[:,8]==3) 
