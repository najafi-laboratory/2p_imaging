...
from utils import get_stim_seq_num_idx
...
class plotter_utils(utils_basic):
    ...
    def __init__(
        ...
        self.list_stim_seq_num = [
            get_stim_seq_num_idx(sl) for sl in self.list_stim_labels]
    ...
    def plot_cluster_oddball_fix_all(self, axs, cate, stim_num):
        ...
        @show_resource_usage
        def plot_standard_fix_seq(ax, seq):
            # collect data.
            [_, [neu_seq, _, stim_seq, _],
             neu_labels, _] = get_neu_trial(
                self.alignment, self.list_labels, self.list_stim_labels,
                trial_idx=[l[seq] for l in self.list_stim_seq_num],
                trial_param=[[2,3,4,5], None, [0], None, [0], [0]],
                cate=cate, roi_id=None)
            # get response within cluster.
            #print('shape: ', neu_seq.shape)
            neu_mean, neu_sem = get_mean_sem_cluster(neu_seq, self.n_clusters, cluster_id)
            
            #print(neu_mean.shape)
            norm_params = [get_norm01_params(neu_mean[i,:]) for i in range(self.n_clusters)]
            # plot results.
            self.plot_cluster_mean_sem(
                ax, neu_mean, neu_sem,
        ...
        try: plot_standard_fix_seq(axs[1], stim_num)
        except: traceback.print_exc()
    ...
    def plot_cluster_oddball_fix_heatmap_all(self, axs, cate, num_seq):
        xlim = [-2500, 6000]
        l_idx, r_idx = get_frame_idx_from_time(self.alignment['neu_time'], 0, xlim[0], xlim[1])
        kernel_all = get_glm_cate(self.glm, self.list_labels, cate)
        cluster_id, neu_labels = get_cluster_cate(self.cluster_id, self.list_labels, cate)
        ...
        @show_resource_usage
        def plot_standard_fix(ax, num_seq):
            # collect data.
            
            [[_, _, _, cmap],
             [neu_seq, _, stim_seq, _], _, _] = get_neu_trial(
                self.alignment, self.list_labels, self.list_stim_labels,
                trial_idx=[l[num_seq] for l in self.list_stim_seq_num],
                trial_param=[None, None, [0], None, [0], [0]],
                cate=cate, roi_id=None)
            c_idx = stim_seq.shape[0]//2
        ...
        try: plot_standard_fix(axs[3], num_seq)
        except: traceback.print_exc()
...
class plotter_main(plotter_utils):
    ...
    def cluster_oddball_fix_all(self, axs_all, stim_num):
        for cate, axs in zip(self.cate_list, axs_all):
            try:
                ...
                self.plot_cluster_oddball_fix_all(axs, cate=cate, stim_num = stim_num)
    ...
    def cluster_oddball_fix_heatmap_all(self, axs_all):
        for cate, axs in zip(self.cate_list, axs_all):
            try:
                ...
                self.plot_cluster_oddball_fix_heatmap_all(axs, cate=cate, num_seq = 1)
