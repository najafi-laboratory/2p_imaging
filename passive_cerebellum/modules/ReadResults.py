...
# read dff traces.
def read_dff(ops):
    mm_path, file_path = get_memmap_path(ops, 'qc_results\\dff.h5')
    with h5py.File(file_path, 'r') as f:
        #dff = create_memmap(f['dff'], 'float32', os.path.join(mm_path, 'dff.mmap'))
        if 'dff' in f.keys():
            dff = create_memmap(f['dff'], 'float32', os.path.join(mm_path, 'dff.mmap'))
        elif 'name' in f.keys():
            dff = create_memmap(f['name'], 'float32', os.path.join(mm_path, 'dff.mmap'))
...
# read dff traces.
def read_spikes(ops):
    mm_path, file_path = get_memmap_path(ops, 'qc_results\\spikes.h5')
    with h5py.File(file_path, 'r') as f:
        #dff = create_memmap(f['dff'], 'float32', os.path.join(mm_path, 'dff.mmap'))
        if 'spikes' in f.keys():
            spikes = create_memmap(f['spikes'], 'float32', os.path.join(mm_path, 'spikes.mmap'))
        elif 'name' in f.keys():
            spikes = create_memmap(f['name'], 'float32', os.path.join(mm_path, 'spikes.mmap'))
...
def read_neural_trials(ops, smooth):
    ...
    with h5py.File(file_path, 'r') as f:
        ...
        neural_trials['spikes']       = create_memmap(f['neural_trials']['spikes'],       'float32', os.path.join(mm_path, 'spikes.mmap'))
...
def read_bpod_mat_data(ops, aud = 0, trialised = 1):
    def _check_keys(d):
        for key in d:
    ...
    img_seq_label_temp = np.array(raw['StimSeqLabel'])
    oddball_types = np.array(raw['OddballTypes'])
    opto_types = np.array(raw['OptoTypes'])
    random_types = np.array(raw['RandomTypes'])
    
    if aud:
        #for arr in [standard_types, fix_jitter_types, img_seq_label, oddball_types, opto_types, random_types]:
        standard_types = np.insert(standard_types, 0, standard_types[0])
        standard_types = np.append(standard_types, standard_types[-1])
        if trialised:
            standard_types = np.insert(standard_types, 0, standard_types[0])
            standard_types = np.repeat(standard_types, 4)
        
        fix_jitter_types = np.insert(fix_jitter_types, 0, fix_jitter_types[0])
        fix_jitter_types = np.append(fix_jitter_types, fix_jitter_types[-1])
        if trialised:
            fix_jitter_types = np.insert(fix_jitter_types, 0, fix_jitter_types[0])
            fix_jitter_types = np.repeat(fix_jitter_types, 4)
            
        img_seq_label = np.insert(img_seq_label_temp, 0, img_seq_label_temp[0])
        img_seq_label = np.append(img_seq_label, img_seq_label[-1])
        if trialised:
            img_seq_label = np.insert(img_seq_label, 0, img_seq_label[0])
            for i in range(len(img_seq_label)):
                if img_seq_label[i] == -1:
                    img_seq_label[i] = img_seq_label[i-1]
            img_seq_label = np.repeat(img_seq_label, 4)
        
        oddball_types = np.insert(oddball_types, 0, oddball_types[0])
        oddball_types = np.append(oddball_types, oddball_types[-1])
        if trialised:
            oddball_types = np.insert(oddball_types, 0, oddball_types[0])
            oddball_types = np.repeat(oddball_types, 4)
        
        opto_types = np.insert(opto_types, 0, opto_types[0])
        opto_types = np.append(opto_types, opto_types[-1])
        if trialised:
            opto_types = np.insert(opto_types, 0, opto_types[0])
            opto_types = np.repeat(opto_types, 4)
        
        random_types = np.insert(random_types, 0, random_types[0])
        random_types = np.append(random_types, random_types[-1])
        
        if trialised:
            random_types = np.insert(random_types, 0, random_types[0])
            random_types = np.repeat(random_types, 4)     
        ...
        img_seq_label = np.append(img_seq_label_temp, img_seq_label_temp[-1])
        if trialised:
            for i in range(len(img_seq_label)):
                if img_seq_label[i] == -1:
                    img_seq_label[i] = img_seq_label[i-1]
...
def read_all(session_config_list, smooth):
    ...
    for i in range(len(session_config_list['list_config'])):
        ...
        list_session_data_path = [
            os.path.join(input_folder , session_config_list['list_config'][i]['session_folder'], n)
