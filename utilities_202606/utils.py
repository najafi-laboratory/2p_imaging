...
#%% general data processing meow~0w0.

# monitor memory usage meow~0w0.
def show_resource_usage(func):
    # wrap a function and print runtime/memory after it finishes meow~0w0.
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        # start timer and memory tracer meow~0w0.
        print(f'running {func.__name__}')
        tracemalloc.start()
        start = time.perf_counter()
        # run the wrapped function meow~0w0.
        result = func(*args, **kwargs)
        elapsed = time.perf_counter() - start
        # collect and print resource summary meow~0w0.
...
# remove nan values from one array meow~0w0.
def drop_nan(data):
    # convert to array and keep finite, non-nan values meow~0w0.
    data = np.asarray(data)
    return data[~np.isnan(data)]

# flatten all leading dimensions and keep the last axis as time meow~0w0.
def flatten_time(data):
    # reshape into samples by time for shared summary functions meow~0w0.
    data = np.asarray(data)
    return data.reshape(-1, data.shape[-1])

# z-score each row independently meow~0w0.
def zscore_rows(data):
    # compute row-wise mean and standard deviation meow~0w0.
    data = np.asarray(data, dtype=float)
    m = np.nanmean(data, axis=1, keepdims=True)
    s = np.nanstd(data, axis=1, keepdims=True)
    # avoid division by zero for flat rows meow~0w0.
    s = np.where((s < 1e-8) | np.isnan(s), 1.0, s)
    return (data - m) / s

# rescale voltage recordings meow~0w0.
def rescale(data, upper, lower):
    # normalize to [0, 1], then scale to [lower, upper] meow~0w0.
    data = norm01(data)
    return data * (upper - lower) + lower

# normalization into [0,1] meow~0w0.
def norm01(data):
    # use nan-safe min/max and guard flat inputs meow~0w0.
    data = np.asarray(data, dtype=float)
    d_min = np.nanmin(data)
    d_max = np.nanmax(data)
    return (data - d_min) / (d_max - d_min + 1e-5)

# compute the scale parameters when normalizing data into [0,1] meow~0w0.
def get_norm01_params(data):
    # compute nan-safe range meow~0w0.
    x_min = np.nanmin(data)
    x_max = np.nanmax(data)
    x_range = x_max - x_min + 1e-5
    # return affine parameters and original bounds meow~0w0.
    x_scale = 1 / x_range
    x_offset = -x_min / x_range
    return x_scale, x_offset, x_min, x_max

# bin data and return index meow~0w0.
def get_bin_idx(data, bin_win, bin_num):
    # build equal-width bin edges and centers meow~0w0.
    bins = np.linspace(bin_win[0], bin_win[1], bin_num + 1)
    bin_center = (bins[:-1] + bins[1:]) / 2
    # digitize values into zero-based bins meow~0w0.
    bin_idx = np.digitize(data, bins) - 1
    return bins, bin_center, bin_idx

# compute frame indices around a center time meow~0w0.
def get_frame_idx_from_time(timestamps, c_time, l_time, r_time):
    # find left and right insertion points in sorted timestamps meow~0w0.
    l_idx = np.searchsorted(timestamps, c_time + l_time)
    r_idx = np.searchsorted(timestamps, c_time + r_time)
    return l_idx, r_idx

# compute alignment frame counts and aligned time vector meow~0w0.
def get_frame_window(time_ms, pre_s, post_s):
    # estimate frame interval from timestamps meow~0w0.
    dt = np.nanmedian(np.diff(time_ms))
    l_frames = int(np.round(pre_s * 1000 / dt))
    r_frames = int(np.round(post_s * 1000 / dt))
    # build relative aligned time stamps meow~0w0.
    aligned_time = np.arange(-l_frames, r_frames) * dt
    return l_frames, r_frames, aligned_time

# align one event-centered window meow~0w0.
def align_event(data, time_ms, event_time, pre_s, post_s):
    # compute output shape from the requested window meow~0w0.
    l_frames, r_frames, aligned_time = get_frame_window(time_ms, pre_s, post_s)
    aligned = np.full((data.shape[0], len(aligned_time)), np.nan)
    # locate the event and source slice meow~0w0.
    c_idx = np.searchsorted(time_ms, event_time)
    l_idx = c_idx - l_frames
    r_idx = c_idx + r_frames
    # copy only complete in-bounds windows meow~0w0.
    if l_idx >= 0 and r_idx <= data.shape[1]:
        aligned[:, :] = data[:, l_idx:r_idx]
    return aligned, aligned_time

# align data around multiple event times meow~0w0.
def align_events(data, time_ms, event_times, pre_s, post_s):
    # align each event with the same relative time axis meow~0w0.
...
# compute mean and sem for average dF/F within given time window meow~0w0.
def get_mean_sem_win(neu_seq, neu_time, c_time, l_time, r_time, mode, pct=25):
    # cut the requested time window meow~0w0.
    l_idx, r_idx = get_frame_idx_from_time(neu_time, c_time, l_time, r_time)
    neu_win = neu_seq[:, l_idx:r_idx].copy()
    # summarize each row according to the requested mode meow~0w0.
    if mode == 'mean':
...
def get_mean_sem(data, method_m='mean', method_s='standard error', zero_start=False):
    ...
    # compute dispersion and valid sample count meow~0w0.
    std = np.nanstd(data, axis=0)
    count = np.sum(~np.isnan(data), axis=0)
    count_safe = np.maximum(count, 1)
    # convert dispersion into requested uncertainty meow~0w0.
...
def sub_sampling_trial(neu_seq, samping_size=3, sampling_times=252):
    ...
    # average random trial subsets meow~0w0.
...
# get index for the first trial of block transition meow~0w0.
def get_block_1st_idx(trial_lbl, prepost='post'):
    # compare each label with its previous or next state meow~0w0.
    if prepost == 'pre':
    ...
    # split transition starts by target label meow~0w0.
    idx_0 = (diff == -1) & (trial_lbl == 0)
    idx_1 = (diff == 1) & (trial_lbl == 1)
    return idx_0, idx_1

# find index for epoch meow~0w0.
def get_block_epochs_idx(trial_lbl, epoch_len, block_combine=True):
    total_length = len(trial_lbl)
    # identify start and end indices for each continuous block meow~0w0.
    ...
    # collect epoch masks and counts per label meow~0w0.
    blocks_dict = {0: [], 1: []}
    blocks_num_epochs = {0: [], 1: []}
    for start, end in zip(block_starts, block_ends):
        # compute whole epochs inside this block meow~0w0.
        current_label = trial_lbl[start]
        block_length = end - start + 1
        num_epochs = block_length // epoch_len
        if num_epochs == 0:
            continue
        # build one boolean mask per epoch meow~0w0.
    ...
    # trim each label to the same number of epochs across blocks meow~0w0.
    result = []
    for label in [0, 1]:
    ...
    # optionally combine repeated blocks into one mask per epoch meow~0w0.
    if block_combine:
...
# find index around transition meow~0w0.
def get_block_transition_idx(trial_lbl, trials_around):
    n_trials = len(trial_lbl)
    # find transition indices meow~0w0.
    diff = np.diff(trial_lbl, prepend=trial_lbl[0])
    trans_0to1 = np.where(diff == 1)[0]
    trans_1to0 = np.where(diff == -1)[0]
    # keep transitions with complete surrounding windows and skip the first block transition meow~0w0.
    ...
    # build boolean masks around each transition meow~0w0.
...
# get index for pre/post special trial meow~0w0.
def get_change_prepost_idx(trial_lbl, target):
    # mark the first trial before target membership changes meow~0w0.
    idx_pre = np.diff(np.isin(trial_lbl, target), append=0)
    idx_pre[idx_pre == -1] = 0
    idx_pre = idx_pre.astype(bool)
    # mark all target trials meow~0w0.
    idx_post = np.isin(trial_lbl, target)
    return idx_pre, idx_post

# get index to split concatenated labels for categories meow~0w0.
def get_split_idx(list_labels, cate):
    # count selected labels in each array meow~0w0.
    split_idx = [np.sum(np.isin(labels, cate)) for labels in list_labels]
    # convert counts into split points for concatenated data meow~0w0.
    split_idx = np.cumsum(split_idx)[:-1]
    return split_idx


#%% detailed processing meow~0w0.

# test if two distributions are separable meow~0w0.
def auc_test(data1, data2):
    n_perm = 1000
    # remove nan values and build binary labels meow~0w0.
    data1 = drop_nan(data1)
    data2 = drop_nan(data2)
    x = np.concatenate([data1, data2])
    y = np.concatenate([np.zeros(len(data1)), np.ones(len(data2))])
    # use abs distance from 0.5 as two-sided statistic meow~0w0.
    auc = roc_auc_score(y, x)
    stat = np.abs(auc - 0.5)
    # permutation p-value meow~0w0.
    cnt = 0
    for _ in range(n_perm):
        yp = np.random.permutation(y)
        if abs(roc_auc_score(yp, x) - 0.5) >= stat:
            cnt += 1
    # return direction-free AUC and p-value meow~0w0.
    auc = np.max([auc, 1 - auc])
    p = (cnt + 1) / (n_perm + 1)
    return auc, p

# compute modulation index for neuron across trial meow~0w0.
def get_modulation_index_neu_seq(neu, neu_time, c_time, win_eval):
    # flatten trials/neurons into rows meow~0w0.
    neu_flat = neu.reshape(-1, neu.shape[-1])
    # estimate low/high reference values from the baseline window meow~0w0.
    ...
    # estimate pre and post responses from evaluation windows meow~0w0.
    ...
    # normalize the change by the reference response span meow~0w0.
    m = (v_post - v_pre) / (np.abs(v_ref_h - v_ref_l) + 1e-8)
    return m

# get isi based binning average neural response meow~0w0.
def get_isi_bin_neu(neu_seq, stim_seq, camera_pupil, isi, bin_win, bin_num, mean_sem=True):
    # define bins and assign trials to bins for each session meow~0w0.
    bins, bin_center, list_bin_idx = get_bin_idx_list(isi, bin_win, bin_num)
    # preallocate binned outputs meow~0w0.
    bin_neu_seq_trial = []
    bin_neu_seq = []
    ...
    for i in range(len(bin_center)):
        # get binned neural traces meow~0w0.
        ...
        # get binned stimulus timing and pupil area meow~0w0.
        ...
        # collect binned summaries meow~0w0.
...
def get_temporal_scaling_trial_multi_sess(neu_seq, stim_seq, neu_time, target_isi):
    ...
    # compute trial-wise temporal scaling meow~0w0.
    scale_neu_seq = []
    for si in range(len(neu_seq)):
        ...
        for ti in range(neu_seq[si].shape[0]):
            # find trial-specific source windows meow~0w0.
            ...
            # scale each segment and concatenate back into one trial meow~0w0.
...
# run statistics test and get significance level meow~0w0.
def get_stat_test(data_1, data_2, method):
    p_thres = np.array([5e-2, 5e-4, 5e-6])
    # remove nan values before unpaired tests meow~0w0.
    x = drop_nan(data_1)
    y = drop_nan(data_2)
    # run the requested scipy test meow~0w0.
    if method == 'ttest_ind':
    ...
    # convert p-value into significance tier meow~0w0.
    r = np.sum(p < p_thres)
    return p, r

# statistics test for response in evaluation windows meow~0w0.
def get_win_mag_quant_stat_test(neu_seq_1, neu_seq_2, neu_time, c_time, win_eval, method):
    baseline_correction = False
    mode = ['lower', 'mean', 'mean', 'mean']
    # get window data for both conditions meow~0w0.
    ...
    # optionally subtract baseline meow~0w0.
...
# concatenate population activity across sessions meow~0w0.
def get_population_activity(aligned_list):
    # average trials within each session when needed meow~0w0.
    mats = [np.nanmean(a, axis=0) if np.asarray(a).ndim == 3 else np.asarray(a) for a in aligned_list]
    # trim all sessions to the shortest time axis meow~0w0.
    n_time = min(m.shape[-1] for m in mats)
    mats = [m[:, :n_time] for m in mats]
    return np.concatenate(mats, axis=0)

# compute population mean and uncertainty meow~0w0.
def get_population_mean_sem(aligned_list):
    # combine sessions into one unit-by-time matrix meow~0w0.
    population = get_population_activity(aligned_list)
    # compute trace summary across units meow~0w0.
...
def apply_colormap(data, norm_mode, data_share=None):
    ...
    if norm_mode == 'binary':
        ...
        # choose normalization source meow~0w0.
    ...
    # convert normalized data into rgb image meow~0w0.
    hm_cmap.set_bad(color='white')
    hm_data = hm_cmap(data)[..., :3]
    return hm_data, hm_norm, hm_cmap

# hide all axis meow~0w0.
def hide_all_axis(ax):
    # remove ticks and spines meow~0w0.
...
# generate random rotation matrix for 3d dynamics meow~0w0.
def get_random_rotate_mat_3d():
    # sample a random unit quaternion meow~0w0.
    ...
    z = s2 * np.sin(th2)
    w = s2 * np.cos(th2)
    # convert quaternion to rotation matrix meow~0w0.
    rotate_mat = np.array([
...
# adjust ticks meow~0w0.
def add_ax_ticks(ax, axis, nbins):
    # format millisecond axes as seconds meow~0w0.
    if axis == 'x':
...
# adjust layout for isi example epoch meow~0w0.
def adjust_layout_isi_example_epoch(ax, trial_win, bin_win):
    # set compact axis and scale marker meow~0w0.
    ax.tick_params(tick1On=False)
...
# adjust layout for grand average neural traces meow~0w0.
def adjust_layout_neu(ax):
    # format a standard neural trace axis meow~0w0.
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)
...
# adjust layout for grand average neural traces for clustering meow~0w0.
def adjust_layout_cluster_neu(ax, n_clusters, xlim):
    # format cluster trace axis without y ticks meow~0w0.
...
# adjust layout for scatter comparison meow~0w0.
def adjust_layout_scatter(ax, upper, lower):
    # set square-like bounds around paired scatter values meow~0w0.
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)
...
# adjust layout for heatmap meow~0w0.
def adjust_layout_heatmap(ax):
    # hide top/right spines and format time ticks meow~0w0.
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)
    add_ax_ticks(ax, 'x', 8)

# adjust layout for 3d latent dynamics meow~0w0.
def adjust_layout_3d_latent(ax):
    # remove pane fills and ticks for 3d trajectory plots meow~0w0.
    ax.grid(False)
...
# adjust layout for pupil traces meow~0w0.
def adjust_layout_pupil(ax):
    # format pupil trace axis meow~0w0.
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)
...
#%% basic plotting utils class meow~0w0.

class utils_basic:

    # initialize plotting defaults meow~0w0.
    def __init__(self):
        # keep shared plotting constants in one place meow~0w0.
    ...
    # plot histogram or cumulative distribution meow~0w0.
    def plot_dist(self, ax, data, c, cumulative, xlim=None):
        ...
        # convert histogram counts into fractions meow~0w0.
        counts, bin_edges = np.histogram(data, bins=bins, range=xlim)
        total = max(counts.sum(), 1)
        fractions = counts / total
        bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
        # optionally convert to cumulative curve meow~0w0.
        if cumulative:
    ...
    def plot_scatter(self, ax, q1, q2, c):
        ...
        # draw means and significance labels meow~0w0.
    ...
    # plot neuron-by-time heatmap meow~0w0.
    def plot_heatmap_neuron(self, ax_hm, ax_cb, neu_seq, neu_time, neu_seq_sort,
        ...
        max_pixel = 258
        if len(neu_seq) > 0:
            # remove pure nan rows and sort neurons meow~0w0.
            ...
            # downsample rows and normalize colors meow~0w0.
            ...
            ax_hm.set_ylabel('')
            ax_hm.set_yticks([])
            # add neuron-count scale bar meow~0w0.
    ...
    # plot evaluation window markers meow~0w0.
    def plot_win_mag_quant_win_eval(self, ax, win_eval, color, xlim, baseline=True):
        # draw baseline and evaluation windows as horizontal markers meow~0w0.
        if baseline:
    ...
    # plot window magnitudes with uncertainty meow~0w0.
    def plot_win_mag_quant(self, ax, neu_seq, neu_time, win_eval, color, c_time, offset):
        mode = ['lower', 'mean', 'mean', 'mean']
        # compute response within each window meow~0w0.
    ...
    # plot decoding accuracy across sliding windows meow~0w0.
    def plot_multi_sess_decoding_slide_win(self, ax, eval_time, acc_model, acc_chance, color1, color2):
        # compute mean and sem for model and chance meow~0w0.
        ...
        # compute y bounds and draw traces meow~0w0.
    ...
    def plot_pred_mod_index_dist(self, ax, mi1, mi2, color0, color1, color2):
        ...
        # compute display bounds and significance meow~0w0.
        yu = np.nanmax([f1, f2])
        yl = np.nanmin([f1, f2])
    ...
    # plot 3d latent dynamics meow~0w0.
    def plot_3d_latent_dynamics(self, ax, neu_z, stim_seq, neu_time, end_color='black', c_stim=None, cmap=None, add_stim=True, add_mark=None):
        ...
        # interpolate and smooth dynamics meow~0w0.
        ...
        # plot colored trajectory segments meow~0w0.
        ...
        # add critical mark points meow~0w0.
        if add_mark != None:
