...
def get_mask_manual(ops):
    masks_npy = np.load(
        os.path.join(ops['save_path0'], 'manual_qc_results', 'masks.npy'),
        allow_pickle=True)
    x1 = ops['xrange'][0]
    x2 = ops['xrange'][1]
    y1 = ops['yrange'][0]
...
def run(ops, diameter):
    ...
    if ops['nchannels'] == 1:
        ...
        save_masks(ops, masks_func, None,None, mean_func, max_func, None, labels)
...
def run_manual(ops, diameter):
    print('===============================================')
    print('===== two channel data roi identification =====')
    print('===============================================')
    print('Reading masks in functional channel')
    [masks_func, mean_func, max_func, mean_anat] = get_mask_manual(ops)
    # deal with data here
    if np.max(masks_func) == 0:
        raise ValueError('No masks found.')
    if ops['nchannels'] == 1:
