...
def read_raw(ops, manual = False):
    if manual:
        F = np.load(
            os.path.join(ops['save_path0'],
                         'qc_results', 'fluo.npy'), allow_pickle=True)
        Fneu = np.load(
            os.path.join(ops['save_path0'],
                         'qc_results', 'neuropil.npy'), allow_pickle=True)
        stat = np.load(
            os.path.join(ops['save_path0'],
                         'qc_results', 'stat.npy'), allow_pickle=True)
    else:
        F = np.load(
            os.path.join(ops['save_path0'],
...
def save_manual_qc_results(
        ops,
        fluo, neuropil, stat, masks, stat_file_name='stat'
):
    if not os.path.exists(os.path.join(ops['save_path0'], 'manual_qc_results')):
        os.makedirs(os.path.join(ops['save_path0'], 'manual_qc_results'))
    np.save(os.path.join(ops['save_path0'], 'manual_qc_results', 'fluo.npy'), fluo)
    np.save(os.path.join(ops['save_path0'],
            'manual_qc_results', 'neuropil.npy'), neuropil)
    np.save(os.path.join(ops['save_path0'],
            'manual_qc_results', f'{stat_file_name}.npy'), stat)
    np.save(os.path.join(ops['save_path0'], 'manual_qc_results', 'masks.npy'), masks)
    np.save(os.path.join(ops['save_path0'], 'ops.npy'), ops)
...
def run_manual(
    ...
    print('===============================================')
    [F, Fneu, stat] = read_raw(ops, manual = True)
    
    good_roi_id, bad_roi_id = read_ROI_label(ops)
    fluo, neuropil, stat = reset_manual_roi(bad_roi_id, F, Fneu, stat)
    print('Saving {} ROIs after manual quality control'.format(fluo.shape[0]))
    masks = stat_to_masks(ops, stat)
    for name in stat_file_names:
        save_manual_qc_results(ops, fluo, neuropil, stat, masks, name)
