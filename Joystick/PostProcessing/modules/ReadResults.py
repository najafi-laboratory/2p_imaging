...
def read_dff(ops, folder = False):
    if folder:
        f = h5py.File(os.path.join(ops['save_path0'], folder + '/dff.h5'), 'r')
    else:
...
def save_lables(ops, data_good, data_bad):
    f = h5py.File(os.path.join(ops['save_path0'], 'ROI_label.h5'), 'w')
    f['good_roi'] = data_good
    f['bad_roi'] = data_bad
    f.close()
