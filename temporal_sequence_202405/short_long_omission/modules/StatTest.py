...
def test_prepost(neu_seq_pre, neu_seq_post, neu_time, win_evoke):
    ...
    for i in range(neu_seq_pre.shape[1]):
        ...
        neu_pre = np.mean(neu_seq_pre[:,i,l_evoke:r_evoke], axis=0)
        neu_post = np.mean(neu_seq_post[:,i,l_evoke:r_evoke], axis=0)
        pvalue = ks_2samp(neu_pre, neu_post)[1]
        responsive[i] = 1 if pvalue < p_thres else 0
    responsive = responsive.astype('bool')
    return responsive

# pick trials and compute responsiveness for normal.
...
def stat_test_change(neu_seq, neu_time, stim_labels):
    win_evoke = [0,250]
    idx_post = pick_trial(stim_labels, [-2,-3,-4,-5], None, None, None, [0])
...
def stat_test_oddball(neu_seq, neu_time, stim_labels):
    ...
    [idx_pre_short, idx_pre_long,
     idx_post_short, idx_post_long] = get_odd_stim_prepost_idx(stim_labels)
    responsive_short = test_prepost(
        neu_seq[idx_pre_short,:,:], neu_seq[idx_post_short,:,:], neu_time, win_evoke)
    responsive_long = test_prepost(
        neu_seq[idx_pre_long,:,:], neu_seq[idx_post_long,:,:], neu_time, win_evoke)
    responsive = responsive_short + responsive_long
    return responsive
...
def run(ops):
    ...
    [neu_seq, neu_time, _, _, _, _, _] = get_stim_response(
            neural_trials, 50, 100, vol=False)
    print('Running statistics test')
    r_normal  = stat_test_normal(neu_seq, neu_time, stim_labels)
    r_change  = stat_test_change(neu_seq, neu_time, stim_labels)
    r_oddball = stat_test_oddball(neu_seq, neu_time, stim_labels)
