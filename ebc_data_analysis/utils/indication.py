...
def intervals(dff_signal, event_index, ap_index, interval_window_led, interval_window_cr, interval_window_ap, interval_window_bl, isi_time, sample_id):
    cr_interval = {}
    led_interval = {}
    ap_interval = {}
    base_line_interval = {}
    ...
    for roi in range(len(dff_signal)):
        ...
        for id in dff_signal[roi]:
            ...
            if event_index[id] < math.floor(interval_window_bl / 33.6) or \
               event_index[id] + math.floor(isi_time / 33.6) + math.floor(interval_window_ap / 33.6) > len(dff_signal[roi][id]):
            ...
            led_interval[roi][id] = dff_signal[roi][id][
                event_index[id]: 
                event_index[id] + math.floor(interval_window_led / 33.6)
            ]
            ...
            base_line_interval[roi][id] = dff_signal[roi][id][
                event_index[id] - math.floor(interval_window_bl / 33.6): 
                event_index[id]
            ]
...
def ttest_intervals(interval_under_test, base_interval):
    t_stat = {}
    p_value = {}
    nan_ids = []
    for roi in range(len(interval_under_test)):
        t_stat[roi] = {}
        p_value[roi] = {}
    for roi in range(len(interval_under_test)):
...
def calculate_average_ttest(t_stat, p_value):
    roi_avg_t_stat = {}
    roi_avg_p_value = {}
    ...
    return roi_avg_t_stat, roi_avg_p_value, overall_avg_t_stat, overall_avg_p_value

def sort_rois_by_ttest(roi_avg_t_stat, top_n=10):
...
def sort_dff_avg(dff, event_index, ap_index):
    avg_cr = {}
    cr_window = {}
    for roi in dff:
        cr_window[roi] = dff[roi][event_index:ap_index]
        avg_cr[roi] = np.mean(cr_window[roi])
    sorted_roi = {roi: avg_cr[roi] for roi in sorted(avg_cr, key=avg_cr.get, reverse=True)}
    return sorted_roi

def sort_dff_max(dff, event_index, ap_index):
    max_cr = {}
    cr_window = {}
    for roi in dff:
        cr_window[roi] = dff[roi][event_index:ap_index]
