...
def label_block_halves(block_type):
    """
    Separate each block into early (0) and late (1) halves.
    
    Parameters:
    -----------
    block_type : array-like
        Array where 1 = short block, 2 = long block, 0 = ignore
    
    Returns:
    --------
    labels : numpy array
        Array of same length as block_type with:
        - 0 = early half of block
        - 1 = late half of block  
        - -1 = ignored trials (where block_type == 0)
    """
    
    block_type = np.array(block_type)
    labels = np.full_like(block_type, -1)  # Initialize with -1 (ignored)
    
    # Find block boundaries
    valid_mask = (block_type == 1) | (block_type == 2)
    valid_indices = np.where(valid_mask)[0]
    
    if len(valid_indices) == 0:
        return labels
    
    # Find transitions (including start and end)
    block_changes = np.diff(block_type[valid_indices])
    transition_points = valid_indices[1:][block_changes != 0]
    
    # Add start and end points
    block_boundaries = np.concatenate([[valid_indices[0]], transition_points, [valid_indices[-1] + 1]])
    ...
    for i in range(len(block_boundaries) - 1):
        ...
        if block_length < 2:
            # Single trial blocks - label as early (0)
            labels[start_idx:end_idx] = 0
        else:
            # Split into halves
            mid_point = start_idx + block_length // 2
            
            # Early half (first half)
            labels[start_idx:mid_point] = 0
            
            # Late half (second half)  
            labels[mid_point:end_idx] = 1
    
    return labels

def decoder_decision(neural_trials, labels, decoding, l_frames=60, r_frames=120, 
    ...
    if decoding == 'epoch':
        ...
        labels_decoder = label_block_halves(block_type)
