...
class plotter_utils(utils):
        ...
        if not np.isnan(np.sum(self.neu_seq_lick)):
            ...
            if roi_id != None:
                neu_cate = np.expand_dims(
                    self.neu_seq_lick[:, roi_id, :], axis=1)
                _, color1, color2, _ = get_roi_label_color(self.labels, roi_id)
            mean_spont, sem_spont = get_mean_sem(
                neu_cate[self.lick_label == 0, :, :])
            mean_consume, sem_consume = get_mean_sem(
                neu_cate[self.lick_label == 1, :, :])
            self.plot_mean_sem(ax, self.neu_time_lick,
                               mean_spont, sem_spont, color1, 'spont')
            self.plot_mean_sem(ax, self.neu_time_lick,
                               mean_consume, sem_consume, color2, 'consume')
            upper = np.max([mean_spont, mean_consume]) + \
                np.max([sem_spont, sem_consume])
            lower = np.min([mean_spont, mean_consume]) - \
                np.max([sem_spont, sem_consume])
    ...
    def plot_push_onset(
        ...
        if roi_id != None:
            neu_push1 = np.expand_dims(
                self.neu_seq_push1[:, roi_id, :], axis=1)
            neu_push2 = np.expand_dims(
                self.neu_seq_push2[:, roi_id, :], axis=1)
            _, color1, _, _ = get_roi_label_color(self.labels, roi_id)
        if not np.isnan(np.sum(neu_push1)) and not np.isnan(np.sum(neu_push2)):
            m_push1_all,    s_push1_all = get_mean_sem(
                neu_push1[self.outcome_push1 != 1, :, :])
            m_push2_reward, s_push2_reward = get_mean_sem(
                neu_push2[self.outcome_push2 == 0, :, :])
            m_push2_punish, s_push2_punish = get_mean_sem(
                neu_push2[self.outcome_push2 == 3, :, :])
            ax.axvline(0, color='grey', lw=1,
                       label='PushOnset', linestyle='--')
            self.plot_mean_sem(
