...
def plot_VIPTD_G8_example_traces(
    ...
    sub_dff_exc = sub_dff_exc[:np.min(
        [num_exc, sub_dff_exc.shape[0]]), time_img_idx]
    sub_dff_inh = dff[labels == -1, :]
    sub_dff_inh = sub_dff_inh[:np.min(
        [num_inh, sub_dff_inh.shape[0]]), time_img_idx]
...
def plot_roi_raw_trace(
    ...
    for i in range(len(axs)):
        ...
        axs[i].set_xticklabels(
            (i*max_ms/60000 + np.arange(0, max_ms/60000+1))*60)
