...
class plotter_utils(utils):
    ...
    def __init__(self, neural_trials, labels, significance, cate_delay):
        ...
        l_idx, r_idx = get_frame_idx_from_time(
            neu_time, 0, self.win[0], self.win[1])
        ...
        self.delay_all = [
            ...
            get_trial_type(self.cate_delay, self.delay_push1,
                           1).astype('int8'),
            get_trial_type(self.cate_delay, self.delay_retract1,
                           1).astype('int8'),
            get_trial_type(self.cate_delay, self.delay_vis2, 1).astype('int8'),
            get_trial_type(self.cate_delay, self.delay_wait2,
                           1).astype('int8'),
            get_trial_type(self.cate_delay, self.delay_push2,
                           1).astype('int8'),
            get_trial_type(self.cate_delay, self.delay_reward,
                           1).astype('int8'),
            get_trial_type(self.cate_delay, self.delay_retract2,
                           1).astype('int8'),
    ...
    def plot_low_dynamics(self, ax, x, y, outcome, class_labels, reward_only=False, cate=None):
        ...
        z = model.fit_transform(
            x.reshape(-1, dim[-1])).reshape(dim[0], dim[1], 2)
        ax.plot(np.mean(z[y == 0, :, 0], axis=0), np.mean(
            z[y == 0, :, 1], axis=0), color=color1)
        ax.plot(np.mean(z[y == 1, :, 0], axis=0), np.mean(
            z[y == 1, :, 1], axis=0), color=color2)
        ax.scatter(np.mean(z[y == 0, :, 0], axis=0)[-1],
                   np.mean(z[y == 0, :, 1], axis=0)[-1], color=color1)
        ax.scatter(np.mean(z[y == 1, :, 0], axis=0)[-1],
                   np.mean(z[y == 1, :, 1], axis=0)[-1], color=color2)
        ax.tick_params(tick1On=False)
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        ax.set_xlabel('pc1')
    ...
    def all_decode(self, axs):
        ...
        axs[0][0].set_title(
            'population block decoding accuracy (reward) (all)')
...
class plotter_VIPTD_G8_model(plotter_utils):
    ...
    def plot_block_type_population_decoding(self, axs, ep):
        ...
        for i in range(len(self.state_all)):
            if not np.isnan(np.sum(self.neu_seq_all[i])):
                ...
                self.plot_pop_decode_box(
                    ...
                    chance=True, reward_only=reward_only)
                # outcome percentage.
                self.plot_class_outcome_pc(
                    axs[1], delay, outcome, i, reward_only=reward_only)
        ...
        axs[0].set_xlabel('state [{},{}] ms window'.format(
    ...
    def plot_block_epoch_decoding_population(self, axs, block_type):
        ...
        for i in range(len(self.state_all)):
            if not np.isnan(np.sum(self.neu_seq_all[i])):
                ...
                outcome = self.outcome_all[i][idx]
                # excitory.
                x = neu_seq[:, (self.labels == -1) *
                            self.significance[i], :].copy()
                self.plot_pop_decode_box(
                ...
                self.plot_pop_decode_box(
                    axs[0], x, block_tran, outcome, i +
                    self.offset[2], self.c_all,
                    chance=True)
                # outcome percentage.
    ...
    def block_type_population_pca(self, axs):
        ...
        self.plot_state_pca(axs[0], x, y, outcome, [
                            'short', 'long'], reward_only=True, cate=-1)
        axs[0].set_title(
            'PCA of block decoding features at WaitForPush2 (reward) (exc)')
        # inhibitory.
        x = neu_seq[:, (self.labels == 1)*self.significance[state], :].copy()
        self.plot_state_pca(axs[1], x, y, outcome, [
                            'short', 'long'], reward_only=True, cate=1)
        axs[1].set_title(
            'PCA of block decoding features at WaitForPush2 (reward) (inh)')
        # all.
        x = neu_seq[:, self.significance[state], :].copy()
        self.plot_state_pca(axs[2], x, y, outcome, [
                            'short', 'long'], reward_only=True, cate=0)
        axs[2].set_title(
            'PCA of block decoding features at WaitForPush2 (reward) (all)')
    ...
    def block_tran_population_pca(self, axs):
        ...
        x = neu_seq[:, (self.labels == -1)*self.significance[state], :].copy()
        self.plot_state_pca(axs[0], x, y, outcome, [
                            'ep1', 'ep2'], reward_only=True, cate=-1)
        axs[0].set_title(
            'PCA of block epoch decoding features at WaitForPush2 (exc)')
        # inhibitory.
        x = neu_seq[:, (self.labels == 1)*self.significance[state], :].copy()
        self.plot_state_pca(axs[1], x, y, outcome, [
                            'ep1', 'ep2'], reward_only=True, cate=1)
        axs[1].set_title(
            'PCA of block epoch decoding features at WaitForPush2 (inh)')
        # all.
        x = neu_seq[:, self.significance[state], :].copy()
        self.plot_state_pca(axs[2], x, y, outcome, [
                            'ep1', 'ep2'], reward_only=True, cate=0)
        axs[2].set_title(
            'PCA of block epoch decoding features at WaitForPush2 (all)')
    ...
    def block_type_dynamics(self, axs):
        ...
        # excitory.
        x = neu_seq[:, (self.labels == -1)*self.significance[state], :].copy()
        self.plot_low_dynamics(axs[0], x, y, outcome, [
                               'short', 'long'], reward_only=True, cate=-1)
        axs[0].set_title('PCA dynamics since WaitForPush2 (exc)')
        # inhibitory.
        x = neu_seq[:, (self.labels == 1)*self.significance[state], :].copy()
        self.plot_low_dynamics(axs[1], x, y, outcome, [
                               'short', 'long'], reward_only=True, cate=1)
        axs[1].set_title('PCA dynamics since WaitForPush2 (inh)')
        # all.
        x = neu_seq[:, self.significance[state], :].copy()
        self.plot_low_dynamics(axs[2], x, y, outcome, [
                               'short', 'long'], reward_only=True, cate=0)
