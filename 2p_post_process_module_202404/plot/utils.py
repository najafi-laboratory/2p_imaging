...
class utils:
    ...
    def plot_heatmap_neuron(self, ax, neu_seq, neu_time, s):
        if not np.isnan(np.sum(neu_seq)) and len(neu_seq) > 0:
            ...
            smoothed_mean = np.array(
                [np.convolve(row, np.ones(5)/5, mode='same') for row in mean])
            sort_idx_neu = np.argmax(
                smoothed_mean, axis=1).reshape(-1).argsort()
            mean = mean[sort_idx_neu, :].copy()
            heatmap_exc = apply_colormap(
                mean[self.labels[s] == -1, :], cmap_exc)
            heatmap_inh = apply_colormap(
                mean[self.labels[s] == 1, :], cmap_inh)
            ...
            ax.set_yticks([0, int(neu_h.shape[0]/3),
                          int(neu_h.shape[0]*2/3), int(neu_h.shape[0])])
            ax.set_yticklabels(
                [0, int(neu_h.shape[0]/3), int(neu_h.shape[0]*2/3), int(neu_h.shape[0])])
            '''
    ...
    def plot_heatmap_trials(self, ax, neu_seq, neu_time, cmap, norm=True):
        if not np.isnan(np.sum(neu_seq)) and len(neu_seq) > 0:
            ...
            ax.set_xticks([0, zero, len(neu_time)])
            ax.set_xticklabels([int(neu_time[0]), 0, int(neu_time[-1])])
            ax.set_yticks([0, int(mean.shape[0]/3),
                          int(mean.shape[0]*2/3), int(mean.shape[0])])
            ax.set_yticklabels(
                [0, int(mean.shape[0]/3), int(mean.shape[0]*2/3), int(mean.shape[0])])
            cbar = ax.figure.colorbar(img, ax=ax, ticks=[0.2, 0.8], aspect=100)
            cbar.ax.set_ylabel('normalized response',
