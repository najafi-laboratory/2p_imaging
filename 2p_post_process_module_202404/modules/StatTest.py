...
def test_win(neu_seq, neu_time, win_base, win_early):
    ...
    for i in range(neu_seq.shape[1]):
        l_base, r_base = get_frame_idx_from_time(
            neu_time, win_base[0], win_base[1])
        l_early, r_early = get_frame_idx_from_time(
            neu_time, win_early[0], win_early[1])
...
def run(ops):
    ...
    r_push = test_win(neu_seq_push1,    neu_time_push1,
                      [-500, 0], [50, 250])
    r_retract = test_win(
        neu_seq_retract1, neu_time_retract1, [-500, 0], [50, 250])
    r_wait = test_win(neu_seq_wait2,    neu_time_wait2,
                      [-500, 0], [50, 250])
    r_reward = test_win(neu_seq_reward,   neu_time_reward,
                        [-500, 0], [50, 250])
    r_punish = test_win(neu_seq_punish,   neu_time_punish,
                        [-500, 0], [50, 250])
    r_lick = test_win(neu_seq_lick,     neu_time_lick,
                      [-500, 0], [50, 250])
