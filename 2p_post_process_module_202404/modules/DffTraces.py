...
def run(
        ...
        preproc_baseline_params = {'lam_range':[1e3], 
                                   'p_range': [0.01], 
                                   'n_iter': 10, 
                                   'njobs': -1, 
                                   'scoring': 'l2'},
    ...
    if isinstance(tau, float):
        spikes, uptime = SpikeDeconv.run(ops=ops, dff=filtered_dff, oasis_tau=tau)
    ...
    postprocessor = Postprocessor(corrected_dff=baselined_dff, spikes=spikes)
    postprocessor()
    ...
    print(dff.shape)
    print(baselined_dff.shape)
    print(filtered_dff.shape)
    print(spikes.shape)
    print(uptime.shape)
    print(normalized_spikes.shape)
    print(denoised_spikes.shape)
    print(stas.shape)
    print(multi_tau)
